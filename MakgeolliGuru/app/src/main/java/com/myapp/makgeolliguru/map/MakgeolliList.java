...
public class MakgeolliList {
    ...
    public String deleteDataFromString(String[] newMakgeolli) {
        ...
        if (this.makgeolliListString.contains(info))
            this.makgeolliListString = this.makgeolliListString.replace(info, "");
        else
            info = Arrays.toString(newMakgeolli).replace("[", "").replace("]", "");
            if (this.makgeolliListString.contains(info))
                this.makgeolliListString = this.makgeolliListString.replace(info, "");
            else
                Log.d("deleteDataFromString", "info not found");
